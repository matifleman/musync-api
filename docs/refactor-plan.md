...
- [x] Refresh tokens never expire, aren't revocable (no logout/revoke endpoint), and only one is stored per user (logging in on a second device silently kills the first device's session).
